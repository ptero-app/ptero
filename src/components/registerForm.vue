...
creds.$subscribe(
  (_mut, state) => {
    lf.setItem('credentials', JSON.stringify(state.credentials)).catch((err) => {
      console.log(err)
    })
  },
  { detached: true }
...
<template>
  ...
  <p>
    Credentials are only stored on your device, and not transmitted anywhere other than the servers
    you specify.
  </p>
