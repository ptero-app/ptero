...
function deletePosts() {
  ...
  const filteredCreds: Credential[] = creds.credentials.filter(
    (cred) => cred.username == selectedCreds.value
  ) as Credential[]
  ...
  deleter
    .postsToDelete(numDays.value)
    .then((posts) => {
      const deleteThem = confirm(
        `You are about to delete ${posts.length} posts. This is irreversable. Do you wish to continue?`
      )

      if (deleteThem) {
        statusMessage.value = 'Deleting posts...'
        deleter
          .deletePosts(numDays.value)
          .then((wereDeleted) => {
            if (wereDeleted) {
              statusMessage.value = `Deleted ${posts.length} posts`
            } else {
              statusMessage.value =
                'Call to delete posts did not error, but did not succeed either??? idk try again???'
            }
          })
          .catch((e) => {
...
function deleteReposts() {
  ...
  const deleter = new Deleter(cred)
  deleter
    .repostsToDelete(numDays.value)
    .then((reposts) => {
      const deleteThem = confirm(
        `You are about to delete ${reposts.length} reposts. This is irreversable. Do you wish to continue?`
      )

      if (deleteThem) {
        statusMessage.value = 'Deleting Reposts...'
        deleter
          .deleteReposts(numDays.value)
          .then((wereDeleted) => {
            if (wereDeleted) {
              statusMessage.value = `Deleted ${reposts.length} reposts`
            } else {
              statusMessage.value =
                'Call to delete reposts did not error, but did not succeed either??? idk try again???'
            }
          })
          .catch((e) => {
            statusMessage.value = `Error deleting:\n${e}`
...
<template>
  ...
  <label for="numDays"><strong>Number of days to keep:</strong></label
  ><br />
